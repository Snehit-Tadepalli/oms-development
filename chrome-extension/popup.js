...
class OutputGenerator {
  ...
  static async forVersion1(output) {
    ...
    if (output.type === "relation") {
      ...
      const scrapperOutput = await OutputGenerator.scrapper(+output.id);
      ...
      return `${output.type}/${output.id},${country} is of v${output.version} Status is unknown`;
    }
  }

  static async scrapper(id) {
    return -1;
  }

  static async distance(origin, destination) {
  ...
  static async forVersionGreaterThan1(output) {
    ...
    if (output.type === "relation") {
      ...
      const scrapperOutput = await Promise.race([
        OutputGenerator.scrapper(+output.id),
      ...
      return `${output.type}/${output.id},${country} is of v${output.version}, ${tagAndMemberChanges} Status is unknown`;
    }
    ...
    if (output.type === "node") {
      ...
      const distanceCalc = await OutputGenerator.distance(origin, destination);
